...
class ActionProvider {
  ...
  async handleFetchUpcomingEvents() {
    ...
    const upcomingEvents = [
      { title: "Konza Greening", date: "May 25, 2024" },
      { title: "Konza Marathon", date: "May 3, 2024" },

    ];

    this.updateChatWithMessage("Here are the upcoming events:", {
  ...
  handleSpeakerInformation() {
    const speakerInfoMessage =
      "We feature a range of Guest of honors from industry leaders to local experts.";
    this.updateChatWithMessage(speakerInfoMessage);
  }

  handleSponsorshipOpportunities() {
    const sponsorshipMessage =
      "For sponsorship inquiries, please contact our marketing team at marketing@konza.go.ke";
  ...
  handleKonzaEventsInfo() {
    this.updateChatWithMessage(
      "Konza Events is an innovative web application designed to revolutionize event management and booking experiences."
    );
  }

  handleKonzaEventsPurpose() {
    this.updateChatWithMessage(
      "The purpose of KonzaEvents web application is to offer users a seamless platform for event management, booking, and customization, providing a user-friendly experience for planning and attending various events."
    );
  }
  ...
  handleCustomerSupport() {
    this.updateChatWithMessage(
      "Our customer support team is here to help you. You can reach us through the Contact Us page or by emailing contact@KonzaEvents.com"
    );
  }
}
